...
export function ListingPerformanceReviewPanel({ rows, t }: { rows: PlainRecord[]; t: EvidenceTranslator }) {
  ...
  return (
    <section aria-label={t('Listing Performance Review')}>
      <Card title={t('Listing Performance Review')} size="small">
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Alert
            type="info"
            showIcon
            message={t('Non-action review surface')}
            description={t('Listing evidence only · 0 family actions created')}
          />
          <fieldset>
          ...
          {filteredRows.length ? (
            <Space wrap aria-label={t('Published listing performance evidence')}>
              {filteredRows.map((row) => {
                ...
                return (
                  ...
                  >
                    {text(row.asin)} · {text(row.sku)}
                  </button>
                );
              })}
            </Space>
          ) : (
            <Typography.Text type="secondary">{t('No matching listing evidence.')}</Typography.Text>
          )}
          {selectedRow ? <CorrectedInventoryEvidencePanel row={selectedRow} t={t} /> : null}
        </Space>
      </Card>
    </section>
  );
