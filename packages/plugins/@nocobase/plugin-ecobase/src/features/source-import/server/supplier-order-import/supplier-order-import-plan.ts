...
const REDACTED_TRACKER_HEADERS = new Set([
  'wholesale price list',
  'product catalog',
  'map agreement',
  'pr portal link',
...
export function buildSupplierOrderImportPlan(params: {
  ...
  for (const [code, rows] of [...masterRows.entries()].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    suppliers.push({
      ...
      activeStatus: selectedReader.string('Active Status'),
      reachedVia: selectedReader.string('Reached Via'),
      receivedEmail: selectedReader.string('Recieved Email'),
  ...
  const supplierAccounts: SupplierAccountImportPlanRow[] = [
    ...new Map(
      orders.map((order) => {
        ...
        return [
          key,
          {
            externalSupplierCode: order.externalSupplierCode,
            companyKey: order.companyKey,
            accountName: `${order.externalSupplierCode}:${order.companyKey}`,
            accountType: 'ordering_relationship' as const,
            market: order.sourceMarketplace,
            metadata: { source: 'canonical_purchase_order' as const },
          },
        ];
      }),
    ).values(),
  ...
  const sourceFiles = sources.map((source) => ({
    ...
    headers:
      source.file.role === 'supplier_tracker'
        ? source.headers.filter((header) => !REDACTED_TRACKER_HEADERS.has(normalizeHeader(header)))
        : source.headers,
