...
import { EcobaseDailyOperationsBriefNarrativeService, NarrativeGroundingValidator, type EcoNarrativeProvider } from '../services/daily-operations-brief-narrative-service';
...
describe('Ecobase report service', () => {
  ...
  it('generates idempotent daily operations brief evidence without secrets', async () => {
    const db = new MemoryDatabase();
    await db.getRepository(ECOBASE_COLLECTIONS.sourceConnections).create({ values: { id: 'source-brief-1', name: 'Sellerboard Brief', sourceType: 'sellerboard', domain: 'amazon_operations', active: true, required: true, freshnessSlaMinutes: 1440, secretRef: 'secret://sellerboard', config: { company: 'ACME', token: 'do-not-leak', warningPolicy: { required: true } } } });
    await db.getRepository(ECOBASE_COLLECTIONS.importRuns).create({ values: { id: 'run-brief-1', sourceConnectionId: 'source-brief-1', adapterName: 'sellerboard_csv', sourceIdentifier: 'brief', sourceVersion: '2026-06-01', idempotencyKey: 'brief-run-1', status: 'success', rowCount: 20, normalizedCount: 20, warningCount: 0, startedAt: '2026-06-01T08:00:00.000Z', finishedAt: '2026-06-01T08:01:00.000Z' } });
    await db.getRepository(ECOBASE_COLLECTIONS.planningProducts).create({ values: { id: 'product-brief-1', naturalKey: 'ACME:B00BRIEF', company: 'ACME', canonicalAsin: 'B00BRIEF', title: 'Brief product', auditSummary: { source: 'inventory_planning_fallback' } } });
    await db.getRepository(ECOBASE_COLLECTIONS.inventorySnapshots).create({ values: { naturalKey: 'inventory-brief-1', sourceConnectionId: 'source-brief-1', planningProductId: 'product-brief-1', snapshotDate: '2026-06-10', company: 'ACME', asin: 'B00BRIEF', sku: 'SKU-BRIEF', stock: 0, reserved: 1, inbound: 0, ordered: 0, prepStock: 0, salesVelocity: 3, payload: {} } });
    await db.getRepository(ECOBASE_COLLECTIONS.planningParameters).create({ values: { naturalKey: 'parameter-brief-1', sourceConnectionId: 'source-brief-1', planningProductId: 'product-brief-1', company: 'ACME', asin: 'B00BRIEF', sku: 'SKU-BRIEF', supplier: 'Brief Supplier', supplierId: 'supplier-brief-1', leadTimeDays: 24, confirmedAt: '2026-04-01T00:00:00.000Z', profitPerUnit: 10, payload: {} } });
    await db.getRepository(ECOBASE_COLLECTIONS.suppliers).create({ values: { id: 'supplier-brief-1', naturalKey: 'supplier:ACME:brief', company: 'ACME', name: 'Brief Supplier' } });
    await db.getRepository(ECOBASE_COLLECTIONS.supplierProductLinks).create({ values: { id: 'supplier-link-1', company: 'ACME', planningProductId: 'product-brief-1', supplierId: 'supplier-brief-1', supplierName: 'Brief Supplier', role: 'preferred', confidence: 'high', active: true, lastSeenAt: '2026-06-01T00:00:00.000Z', payload: {} } });
    await db.getRepository(ECOBASE_COLLECTIONS.supplierLeadTimes).create({ values: { naturalKey: 'lead-time-brief-1', company: 'ACME', supplierId: 'supplier-brief-1', supplierName: 'Brief Supplier', planningProductId: 'product-brief-1', asin: 'B00BRIEF', sku: 'SKU-BRIEF', scope: 'product', leadTimeDays: 24, confirmedAt: '2026-04-01T00:00:00.000Z' } });
    await db.getRepository(ECOBASE_COLLECTIONS.planningCalculationSnapshots).create({ values: { naturalKey: 'calc-brief-1', planningProductId: 'product-brief-1', calculationDate: '2026-06-10', company: 'ACME', canonicalAsin: 'B00BRIEF', tier: 'A', sellableStock: 0, pipelineStock: 0, daysOfCover: 0, salesVelocity: 3, leadTimeDays: 24, restockDeadlineImproved: '2026-06-01', oosDate: '2026-06-11', estimatedProfitRisk: 750, calculationStatus: 'complete', dataCompleteness: 'complete' } });
    await db.getRepository(ECOBASE_COLLECTIONS.supplierOrders).create({ values: { id: 'order-brief-1', company: 'ACME', supplierId: 'supplier-brief-1', supplierName: 'Brief Supplier', externalOrderRef: 'PO-BRIEF-1', status: 'payment_pending', lastMeaningfulUpdateAt: '2026-06-09T00:00:00.000Z' } });
    await db.getRepository(ECOBASE_COLLECTIONS.supplierOrderLines).create({ values: { id: 'line-brief-1', company: 'ACME', supplierOrderId: 'order-brief-1', planningProductId: 'product-brief-1', asin: 'B00BRIEF', sku: 'SKU-BRIEF', orderedQty: 20, receivedQty: 0, expectedSellableDate: '2026-06-20', status: 'payment_pending', observedAt: '2026-06-09T00:00:00.000Z' } });
    ...
    expect(body.data).toMatchObject({
      ...
      evidencePack: expect.objectContaining({
        summaryCounts: expect.objectContaining({ inventoryRiskCount: 1, includedInventoryRiskCount: 1, supplierOrderContextCount: 1 }),
        inventoryRisks: [expect.objectContaining({ asin: 'B00BRIEF', velocityPerDay: 3, estimatedProfitRisk: expect.any(Number), supplierOrderState: 'placed_not_purchased' })],
        supplierOrderContext: [expect.objectContaining({ externalOrderRef: 'PO-BRIEF-1', coverageState: 'payment_pending', isTrustedCoverage: false, openQty: 20 })],
        leadTimeIssues: [expect.objectContaining({ asin: 'B00BRIEF', actionRequired: 'refresh_stale_lead_time' })],
      }),
    });
  ...
  it('generates and persists an Eco daily operations brief narrative from bounded evidence', async () => {
    ...
    const provider: EcoNarrativeProvider = {
      async generate(input) {
        ...
        return JSON.stringify({
          ...
          bodyMarkdown: `# Ecobase Daily Operations Brief — ${request.evidencePack.date}\n\nNo major exception needs action today. Cited evidence: ${noActionEvidenceId}.`,
    ...
    const result = await new EcobaseDailyOperationsBriefNarrativeService(db, provider).generateBrief({ date: '2026-06-10', company: 'ACME' });

    expect(result).toMatchObject({ status: 'ready_to_send', validationStatus: 'passed', subject: 'Ecobase daily brief: no major exceptions' });
    expect(result.bodyHtml).toContain('<h1>Ecobase Daily Operations Brief');
    const run = await db.getRepository(ECOBASE_COLLECTIONS.reportRuns).findOne({ filterByTk: result.reportRunId });
    expect(run).toMatchObject({ status: 'ready_to_send', validationStatus: 'passed', deliveryStatus: 'preview_ready', subject: result.subject });
  ...
  it('persists provider failures as blocked report runs without deterministic fallback', async () => {
    ...
    expect(result).toEqual(expect.objectContaining({ status: 'blocked_ai_provider_unavailable', validationStatus: 'failed', validationErrors: ['provider rate limited'] }));
    const run = await db.getRepository(ECOBASE_COLLECTIONS.reportRuns).findOne({ filterByTk: result.reportRunId });
    expect(run).toMatchObject({ status: 'blocked_ai_provider_unavailable', deliveryStatus: 'blocked', emailStatus: 'blocked' });
  });

  it('marks daily operations brief delivery sent and failed without creating duplicate runs', async () => {
    const db = new MemoryDatabase();
    await db.getRepository(ECOBASE_COLLECTIONS.reportRuns).create({ values: {
      id: 'report-ready-1',
      company: 'ACME',
      frequency: 'daily',
    ...
    const sent = await service.markSent({ reportRunId: 'report-ready-1', deliveryProvider: 'nocobase-email', messageId: 'message-1' });
    const failed = await service.markFailed({ reportRunId: 'report-failed-1', error: 'SMTP configuration missing' });

    expect(sent).toMatchObject({ deliveryStatus: 'sent', emailStatus: 'sent', deliveryProvider: 'nocobase-email', deliveryMessageId: 'message-1' });
    expect(failed).toMatchObject({ deliveryStatus: 'send_failed', emailStatus: 'send_failed', deliveryError: 'SMTP configuration missing' });
  ...
  it('blocks workflow-send generation when no recipient is configured', async () => {
    const db = new MemoryDatabase();
    const provider: EcoNarrativeProvider = { async generate() { return '{}'; } };

    const result = await new EcobaseDailyOperationsBriefNarrativeService(db, provider).generateBrief({ date: '2026-06-10', mode: 'workflow_send' });
  ...
  it('requires Eco to mention stale or missing-data warnings in the narrative', () => {
    const pack: DailyEvidencePack = {
      ...
      leadTimeIssues: [],
      dataWarnings: [{ evidenceId: 'warning-1', code: 'source_stale', message: 'Sellerboard source is stale.', severity: 'warning', metadata: {} }],
      omissions: [],
      assumptions: [],
    };
    ...
    expect(result).toEqual({ validationStatus: 'failed', validationErrors: ['Narrative omitted required data warning source_stale.'] });
  });
});
