...
class MemoryRepository implements EcobaseRepository {
  ...
  async find(
    ...
    return this.sort(this.filter(params), params.sort).slice(0, params.limit ?? this.records.length);
  }
  async findOne(
    params: { filter?: Record<string, unknown>; filterByTk?: string | number; sort?: string[]; limit?: number } = {},
  ) {
  ...
  async update({
    ...
    const rows = this.filter({ filter, filterByTk });
    rows.forEach((row) => Object.assign(row, values));
    return rows[0] ?? null;
  }
  private filter(params: { filter?: Record<string, unknown>; filterByTk?: string | number }) {
  ...
  private sort(rows: Record<string, unknown>[], sort: string[] = []) {
    const [first] = sort;
    if (!first) return rows;
    const desc = first.startsWith('-');
    const key = desc ? first.slice(1) : first;
    return [...rows].sort((a, b) => (desc ? -1 : 1) * String(a[key] ?? '').localeCompare(String(b[key] ?? '')));
  }
}
...
async function seed(db: MemoryDatabase) {
  ...
  await db.getRepository(ECOBASE_COLLECTIONS.goldInventoryPlanningRows).create({
    values: {
      id: 'gold-inventory-1',
      refreshRunId: 'ai-published-run',
      planningProductId: 'company-product-ai-1',
      companyProductId: 'company-product-ai-1',
      companyProductFamilyId: 'family:company-product-ai-1',
      companyId: 'company:ACME',
      amazonAccountId: 'account:ACME',
      ...
      calculationEvidence: {
        familyActionSnapshot: {
          familyKey: 'family:company-product-ai-1',
          companyProductFamilyId: 'family:company-product-ai-1',
          companyId: 'company:ACME',
          amazonAccountId: 'account:ACME',
          marketplace: 'Amazon.com',
          ...
          targetCompanyProductId: 'company-product-ai-1',
          targetSelectionEvidence: { source: 'test' },
        },
      },
    },
  });
  await db.getRepository(ECOBASE_COLLECTIONS.goldSupplierAttentionRows).create({
    values: {
      id: 'gold-supplier-1',
      company: 'ACME',
      supplierId: 'supplier-1',
      ...
      asin: 'B00AI',
      sku: 'SKU-AI',
      recommendedAction: 'Follow supplier.',
      attentionReason: 'late_expected_sellable_date',
      moneyAtRisk: 500,
      orderRef: 'PO-AI',
      orderStatus: 'paid',
      expectedSellableDate: '2026-06-12',
  ...
  await db.getRepository(ECOBASE_COLLECTIONS.goldAlerts).create({
    values: {
      id: 'gold-alert-1',
      company: 'ACME',
      severity: 'critical',
  ...
  await db.getRepository(ECOBASE_COLLECTIONS.silverSuppliers).create({
    values: {
      id: 'supplier-1',
      company: 'ACME',
      displayName: 'Supplier AI',
      normalizedName: 'supplier ai',
...
describe('Ecobase AI retrieval service', () => {
  it('maps every Appendix A group to retrieval evidence and stores answers without mutating alerts', async () => {
    ...
    for (const question of questions)
      answers.push(await service.answerQuestion({ question, company: 'ACME', date: '2026-06-05' }));
    ...
    expect(
      answers.every((answer) => answer.response.includes('cannot create, update, or resolve operational records')),
    ).toBe(true);
  ...
  it('exposes public retrieve, answer, and coverage actions', async () => {
    ...
    await seed(db);
    const next = vi.fn();
    const answerContext = actionContext(db, {
      question: 'What are the biggest financial risks?',
      company: 'ACME',
      date: '2026-06-05',
    });
    await createEcobaseAiActions().answer(answerContext, next);
    expect(answerContext.body).toEqual({
      data: expect.objectContaining({ provider: 'ecobase-plugin-retrieval', confidence: 'evidence-backed' }),
    });
    ...
    expect(retrievalContext.body).toEqual({
      data: expect.objectContaining({
        ...
        gold: expect.objectContaining({
          ...
          inventoryPlanningRows: expect.any(Array),
          listingPerformanceRows: [expect.objectContaining({ baselineTier: 'B' })],
          supplierAttentionRows: expect.any(Array),
        }),
    ...
    expect(coverageContext.body).toEqual({
      data: expect.arrayContaining([expect.objectContaining({ group: 'order_management' })]),
    });
  });
});
