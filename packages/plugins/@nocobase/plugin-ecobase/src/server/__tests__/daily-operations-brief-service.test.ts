...
async function seedGoldInventoryRow(db: MemoryDatabase, values: Record<string, unknown>) {
  ...
  await db.getRepository(ECOBASE_COLLECTIONS.goldInventoryPlanningRows).create({
    values: {
      ...
      baselineWeightedProfitPerUnit: values.baselineWeightedProfitPerUnit ?? 1,
      ...
      orderedStock: values.orderedStock ?? 0,
      prepStock: values.prepStock ?? 0,
      inboundStock: values.inboundStock ?? 0,
      awdStock: values.awdStock ?? 0,
      supplierPipelineStock: values.supplierPipelineStock ?? 0,
