...
const fileNames: Record<SupplierOrderSourceRole, string> = {
  ...
  supplier_tracker: 'Supplier Analysis Tracker - Supplier Analysis Tracker.csv',
...
describe('supplier/order import planner', () => {
  it('parses canonical source facts without retention or credential payloads', () => {
    ...
    expect(JSON.stringify(plan)).not.toContain('must-not-import');
    expect(JSON.stringify(plan.sourceFiles)).not.toMatch(
      /Username|PR Portal Link|Wholesale Price List|Sheet Link|"pass"/,
    );
