...
const DOMAIN_TITLES: Record<string, string> = {
  ...
  medallion: 'Bronze and silver data',
...
const COLLECTIONS: CollectionDefinition[] = [
  ...
  {
    domain: 'source_import',
    collectionName: ECOBASE_COLLECTIONS.importRuns,
    title: 'Import runs',
  ...
  {
    domain: 'source_import',
    collectionName: ECOBASE_COLLECTIONS.sourceAccessAudits,
    title: 'Source access audits',
    access: 'read_only_audit',
    sourceScoped: true,
  },
  {
    domain: 'source_import',
    collectionName: ECOBASE_COLLECTIONS.sourceWarningPolicies,
    title: 'Source warning policies',
    access: 'configuration',
  },
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.bronzeSourceFiles,
  ...
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverCompanies,
    title: 'Silver companies',
    access: 'read_only_audit',
  },
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverAmazonAccounts,
    title: 'Silver Amazon accounts',
    access: 'read_only_audit',
  },
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverProducts,
    title: 'Silver products',
    access: 'read_only_audit',
  },
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverCompanyProducts,
  ...
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverListingDailyFacts,
    title: 'Silver listing daily facts',
    access: 'read_only_audit',
  },
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverTrafficSnapshots,
    title: 'Silver traffic snapshots',
    access: 'read_only_audit',
  },
  {
    domain: 'medallion',
    collectionName: ECOBASE_COLLECTIONS.silverNormalizationLinks,
  ...
  {
    domain: 'suppliers_orders',
    collectionName: ECOBASE_COLLECTIONS.supplierOrderSettings,
    title: 'Supplier order settings',
    access: 'configuration',
  },
  {
    domain: 'alerts',
    collectionName: ECOBASE_COLLECTIONS.ruleVersions,
    title: 'Rule versions',
    access: 'read_only_audit',
  },
  {
    domain: 'alerts',
    collectionName: ECOBASE_COLLECTIONS.alertEvaluations,
    title: 'Alert evaluations',
    access: 'read_only_audit',
    companyScoped: true,
  },
  {
  ...
  {
    domain: 'reports',
    collectionName: ECOBASE_COLLECTIONS.reportRuns,
    title: 'Report runs',
    access: 'read_only_audit',
    companyScoped: true,
  },
  {
    domain: 'reports',
    collectionName: ECOBASE_COLLECTIONS.reportItems,
    title: 'Report items',
    access: 'read_only_audit',
    companyScoped: true,
  },
  {
    domain: 'ai',
    collectionName: ECOBASE_COLLECTIONS.aiAnswers,
    title: 'AI answers',
    access: 'read_only_audit',
    companyScoped: true,
  },
  {
    domain: 'accuracy',
    collectionName: ECOBASE_COLLECTIONS.dataQualitySignoffs,
    title: 'Data-quality sign-offs',
    access: 'operator_editable',
    companyScoped: true,
  },
  {
    domain: 'accuracy',
    collectionName: ECOBASE_COLLECTIONS.benchmarkFixtures,
    title: 'Benchmark fixtures',
    access: 'configuration',
  },
  {
    domain: 'accuracy',
    collectionName: ECOBASE_COLLECTIONS.accuracyEvaluationRuns,
    title: 'Accuracy evaluation runs',
    access: 'read_only_audit',
    companyScoped: true,
  },
];
...
const STARTER_VIEWS: BusinessViewDefinition[] = [
  ...
  starterView(
    ...
    'Open high-priority alert queue grouped by company.',
    ['company', 'severity', 'status', 'title', 'primaryRootCauseCode', 'openedAt'],
  ...
  starterView(
    ...
    'Report preview items',
    'reports',
    ECOBASE_COLLECTIONS.reportItems,
    'Generated report cards and evidence lines for review.',
    ['company', 'reportRunId', 'section', 'severity', 'title', 'body'],
    {},
    ['company', 'section'],
    ['company', 'section'],
  ),
  starterView(
    ...
    'AI answers with deterministic database evidence.',
    ['company', 'question', 'answerStatus', 'createdAt', 'evidence'],
    {},
    ['company', '-createdAt'],
    ['company'],
  ),
  starterView(
    ...
    'Blocked, stale, or failed source access evidence.',
    ['sourceConnectionId', 'sourceType', 'status', 'blockerCode', 'message', 'createdAt'],
    { status: 'stale' },
    ['-createdAt'],
  ),
];
...
function starterView(
  ...
  return {
    ...
    sort,
    groupBy,
    companyScoped: columns.includes('company'),
    readOnly: true,
  };
}
...
export class EcobaseOperatorWorkspaceService {
  ...
  async getWorkspace(filters: PlainRecord = {}) {
    ...
    const collectionSummaries = await Promise.all(
      COLLECTIONS.map(async (definition) => this.collectionSummary(definition, scope)),
    );

    return {
      filters: { company: scope.company, sourceConnectionId: scope.sourceConnectionId },
      scopeRequired: !scope.hasScope,
      scopeMessage: scope.hasScope
        ? null
        : 'Select a company or source connection before opening rows so data from multiple companies is never mixed by default.',
      permissionModel: {
  ...
  async previewView(values: PlainRecord) {
    ...
    const view = viewKey
      ? [...STARTER_VIEWS, ...(await this.savedBusinessViews())].find((candidate) => candidate.key === viewKey)
      : undefined;
    ...
    if (!targetCollection || !definition) {
      throw new Error(
        `Ecobase operator workspace preview failed: collection "${targetCollection ?? 'unknown'}" is not exposed.`,
      );
    }
    ...
    if (!scopedFilter) {
      throw new Error(
        `Ecobase operator workspace preview failed: collection "${targetCollection}" cannot be safely scoped.`,
      );
    }
  ...
  async saveBusinessView(values: PlainRecord) {
    ...
    if (!definition) {
      throw new Error(
        `Ecobase operator workspace save view failed: collection "${collectionName ?? 'unknown'}" is not exposed.`,
      );
    }
    const view: BusinessViewDefinition = {
      ...
      columns: Array.isArray(values.columns)
        ? values.columns.filter((item): item is string => typeof item === 'string')
        : [],
      groupBy: Array.isArray(values.groupBy)
        ? values.groupBy.filter((item): item is string => typeof item === 'string')
        : [],
  ...
  private async resolveScope(filters: PlainRecord): Promise<ScopeContext> {
    ...
    const scopedSources = sourceConnections.filter((source) => {
      ...
      if (
        requestedCompany &&
        (!requestedCompanyId || (companyId !== requestedCompanyId && companyName !== requestedCompanyName))
      ) {
        return false;
      }
    ...
    const sourceIds = new Set(
      scopedSources.map((source) => asString(source.id)).filter((id): id is string => Boolean(id)),
    );
    if (requestedSourceConnectionId && sourceIds.size === 0) {
      sourceIds.add(requestedSourceConnectionId);
    }
    const importRuns = await this.findAll(
    ...
    const importRunIds = new Set(
      scopedImportRuns.map((run) => asString(run.id)).filter((id): id is string => Boolean(id)),
    );
    const sourceAudits = await this.findAll(
      ECOBASE_COLLECTIONS.sourceAccessAudits,
      sourceIds.size > 0 ? { sourceConnectionId: { $in: [...sourceIds] } } : {},
      { limit: 500 },
    );
  ...
  private async collectionSummary(definition: CollectionDefinition, scope: ScopeContext) {
    ...
    return {
      ...
      latestImportRun: latestImportRun
        ...
        : null,
      freshnessStatus: !scope.hasScope
        ? 'scope_required'
        : latestRunStatus === 'success'
          ? 'fresh'
          : latestRunStatus === 'stale'
            ? 'stale'
            : latestRunStatus
              ? 'attention'
              : 'no_imports',
      warningCount,
      starterViewKeys: STARTER_VIEWS.filter((view) => view.collectionName === definition.collectionName).map(
        (view) => view.key,
      ),
    };
  }
  ...
  private async scopedRowCount(definition: CollectionDefinition, scope: ScopeContext) {
    ...
    if (filter) {
      const repository = this.db.getRepository(definition.collectionName) as {
        count?: (params: { filter: PlainRecord }) => Promise<number> | number;
      };
  ...
  private scopedRows(definition: CollectionDefinition, rows: PlainRecord[], scope: ScopeContext) {
    ...
    return rows.filter((row) => {
      ...
      if (definition.collectionName === ECOBASE_COLLECTIONS.sourceConnections) {
        return (
          scope.sourceIds.has(String(row.id ?? '')) ||
          (scope.companyId !== undefined && row.companyId === scope.companyId)
        );
      }
  ...
  private async savedBusinessViews(): Promise<BusinessViewDefinition[]> {
    const rows = await this.findAll(ECOBASE_COLLECTIONS.ruleVersions, {
      ruleType: 'operator_business_view',
      active: true,
    });
    return rows
  ...
  private async count(collectionName: string, filter: PlainRecord = {}) {
    const repository = this.db.getRepository(collectionName) as {
      count?: (params: { filter: PlainRecord }) => Promise<number> | number;
    };
    if (typeof repository.count === 'function') {
      try {
