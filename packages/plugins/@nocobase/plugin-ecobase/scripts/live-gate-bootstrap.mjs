  ...
  if (seedPhaseEnabled('clickup'))
    await runStage(9, 'Reconcile ClickUp status/comments', async () => {
      ...
      if (Number(clickup.workflowDraftCount ?? 0) !== Number(preview.workflowDraftCount ?? 0)) {
        throw new Error(
          `ClickUp workflow draft apply created ${Number(
            clickup.workflowDraftCount ?? 0,
          )} draft(s); dry-run expected ${Number(preview.workflowDraftCount ?? 0)}`,
        );
      }
      ...
      if (Number(postApplyPreview.workflowDraftCount ?? 0) !== 0) {
        throw new Error(
          `ClickUp post-apply dry-run still proposes ${Number(
            postApplyPreview.workflowDraftCount ?? 0,
          )} workflow draft(s)`,
        );
      }
...
function supplierOrderFiles() {
  ...
  const required = ['Supplier IDs.csv', 'Supplier 2026.csv', 'Purchase Orders.csv', 'OrderDetails.csv'];
