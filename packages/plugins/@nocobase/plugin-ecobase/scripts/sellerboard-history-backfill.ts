...
function requiredArgument(prefix: string) {
  ...
  if (!value) throw new Error(`Sellerboard history operation requires ${prefix}<value>.`);
  return value;
}
...
async function signIn(baseUrl: string) {
  ...
  if (!email || !password) {
    throw new Error(
      'Sellerboard history operation requires ECOBASE_REPAIR_TOKEN or explicit ECOBASE_LIVE_GATE_ADMIN_EMAIL and ECOBASE_LIVE_GATE_ADMIN_PASSWORD.',
    );
  }
  ...
  if (!response.ok || typeof body?.data?.token !== 'string') {
    throw new Error(`Sellerboard history sign-in failed with HTTP ${response.status}.`);
  }
...
async function main() {
  ...
  if (modes.length !== 1) {
    throw new Error(
      'Usage: sellerboard-history-backfill.ts --dry-run|--apply|--verify-idempotency --data-root=<path> --source-version=<date> --json=<path>.',
    );
  }
  ...
  const files = readdirSync(historyRoot)
    .filter((name) => name.includes('Dashboard_by_product') && name.endsWith('.csv'))
    .sort()
    .map((name) => ({ name, content: readFileSync(path.join(historyRoot, name), 'utf8') }));
  if (files.length !== 4) throw new Error(`Sellerboard history operation expected four files; found ${files.length}.`);
  ...
  const action = {
    'dry-run': 'previewSellerboardHistoryBackfill',
    apply: 'applySellerboardHistoryBackfill',
    'verify-idempotency': 'verifySellerboardHistoryBackfillIdempotency',
  }[mode];
  ...
  const body = await response.json();
  if (!response.ok) {
    throw new Error(`Sellerboard history ${mode} failed (${response.status}): ${JSON.stringify(body)}`);
  }
  const result = unwrap(body);
  const output = path.resolve(requiredArgument('--json='));
  mkdirSync(path.dirname(output), { recursive: true });
  writeFileSync(output, `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 });
  console.log(`Sellerboard history ${mode}: digest=${result.decisionDigest ?? result.first?.decisionDigest ?? 'n/a'}`);
